...
from ..core.config import ALGORITHM, JWT_ACCESS_TOKEN_SECRET, JWT_ACCESS_TOKEN_EXPIRES_IN, JWT_REFRESH_TOKEN_SECRET, JWT_REFRESH_TOKEN_EXPIRES_IN
...
class AuthService:
    ...
    async def login(self, user: UserPublic) -> Response:
    ...
    async def get_current_user(self, payload: dict) -> UserPublic:
        statement = select(UserPublic).where(UserPublic.id == payload.get("sub"))
    ...
    async def verify_refresh_token(self, refresh_token: str):
        try:
            payload = await self.decode_token(refresh_token, JWT_REFRESH_TOKEN_SECRET)
    ...
    async def decode_token(self, token: str, secret: str) -> dict:
        try:
            return await jwt.decode(token, secret, algorithms=ALGORITHM)
